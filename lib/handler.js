...
handler._url.post = function(data, callback) {
    ...
    if(url && method) {
        ...
        dao.addDataToUrlModel(urlObject, function(statusCode, res) {
...
handler._url.get = function(data, callback) {
    ...
    if(!data.trimmedPath) {
        dao.getAllMonitoredUrl(function(statusCode, res) {
            callback(statusCode, res);
        });
    } else {
        dao.getUrlDetailsById(data.trimmedPath, function(statusCode, res) {
            callback(statusCode, res);
        });
    }
};
...
handler._url.put = function(data, callback) {
    ...
    if(url) {
        ...
        dao.updateUrlData(id,urlObject, function(statusCode, res) {
            callback(statusCode, res);
        })
...
handler._url.delete = function(data, callback) {
    dao.deleteUrl(data.trimmedPath, function(statusCode, res) {
        callback(statusCode, res);
    });
};
